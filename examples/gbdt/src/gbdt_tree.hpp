...
class GBDTTree {
  public:
    ...
    void train(int& ps_key_ptr, std::map<std::string, std::unique_ptr<KVClientTable<float>>>& name_to_kv_table, std::vector<std::vector<float>>& feat_vect_list, std::vector<std::map<std::string, float>>& min_max_feat_list
    ...
    std::vector<Key> push_local_grad_sum(int& ps_key_ptr, std::unique_ptr<KVClientTable<float>> & table, std::vector<float>& grad_vect, std::vector<float>& _push_val_vect);
    bool check_to_stop();
