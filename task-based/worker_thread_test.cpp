...
TEST_F(TestWorkerThread, Init) {
  FakeMailbox mailbox;
  WorkerThread worker_thread(1, &mailbox);
}

TEST_F(TestWorkerThread, StartStop) {
  FakeMailbox mailbox;
  WorkerThread worker_thread(1, &mailbox);
  auto* work_queue = worker_thread.GetWorkQueue();
  worker_thread.Start();

  Message exit_msg;
  exit_msg.meta.recver = worker_thread.GetWorkerId();
...
TEST_F(TestWorkerThread, SendToScheduler) {
  ...
  Message send_msg;
  send_msg.meta.sender = worker_thread.GetWorkerId();
  send_msg.meta.recver = scheduler_thread.GetSchedulerId();
  send_msg.meta.flag = Flag::kExit;
  worker_thread.Send(send_msg);

    Message msg;
    sheduler_queue->WaitAndPop(&msg);
    CHECK(msg.meta.sender == worker_thread.GetWorkerId());
    CHECK(msg.meta.recver == scheduler_thread.GetSchedulerId());
