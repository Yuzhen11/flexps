...
class SchedulerThread : public ThreadTemplate {
 public:
  SchedulerThread(AbstractMailbox* const mailbox) : ThreadTemplate(0, mailbox) {}

  void RegisterWorker(uint32_t worker_id) {
    CHECK(worker_ids_.find(worker_id) == worker_ids_.end()) << "Worker " << worker_id
  ...
  void Main() override {
    while (true) {
      ...
      CHECK(worker_ids_.find(msg.meta.sender) != worker_ids_.end()) << "Received message from unknown thread";
