...
void TestBgWorker() {
  ...
  std::thread worker_thread1([kTestAppThreadId1, kTestModelId, 
      &app_blocker, &downstream_queue, &range_manager, &ready]() {
  ...
  std::thread worker_thread2([kTestAppThreadId2, kTestModelId,
      &app_blocker, &downstream_queue, &range_manager, &ready]() {
    while (!ready) { std::this_thread::yield(); }
